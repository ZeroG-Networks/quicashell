// Examples from RFC 9001, Appendix A.

const std = @import("std");

pub const conn_id = [_]u8{ 0x83, 0x94, 0xc8, 0xf0, 0x3e, 0x51, 0x57, 0x08 };
pub const initial_secret = [_]u8{ 0x7d, 0xb5, 0xdf, 0x06, 0xe7, 0xa6, 0x9e, 0x43, 0x24, 0x96, 0xad, 0xed, 0xb0, 0x08, 0x51, 0x92, 0x35, 0x95, 0x22, 0x15, 0x96, 0xae, 0x2a, 0xe9, 0xfb, 0x81, 0x15, 0xc1, 0xe9, 0xed, 0x0a, 0x44 };
pub const client_initial_secret = [_]u8{ 0xc0, 0x0c, 0xf1, 0x51, 0xca, 0x5b, 0xe0, 0x75, 0xed, 0x0e, 0xbf, 0xb5, 0xc8, 0x03, 0x23, 0xc4, 0x2d, 0x6b, 0x7d, 0xb6, 0x78, 0x81, 0x28, 0x9a, 0xf4, 0x00, 0x8f, 0x1f, 0x6c, 0x35, 0x7a, 0xea };
pub const client_key = [_]u8{ 0x1f, 0x36, 0x96, 0x13, 0xdd, 0x76, 0xd5, 0x46, 0x77, 0x30, 0xef, 0xcb, 0xe3, 0xb1, 0xa2, 0x2d };

pub const client_iv = [_]u8{ 0xfa, 0x04, 0x4b, 0x2f, 0x42, 0xa3, 0xfd, 0x3b, 0x46, 0xfb, 0x25, 0x5c };
pub const client_hp = [_]u8{ 0x9f, 0x50, 0x44, 0x9e, 0x04, 0xa0, 0xe8, 0x10, 0x28, 0x3a, 0x1e, 0x99, 0x33, 0xad, 0xed, 0xd2 };

pub const client_header = [_]u8{ 0xc3, 0x00, 0x00, 0x00, 0x01, 0x08, 0x83, 0x94, 0xc8, 0xf0, 0x3e, 0x51, 0x57, 0x08, 0x00, 0x00, 0x44, 0x9e, 0x00, 0x00, 0x00, 0x02 };
const client_body_hex = "" ++
    ...
    "048000ffff";
pub var client_body: [client_body_hex.len / 2]u8 = undefined;

const client_sample_hex = "" ++
    "d1b1c98dd7689fb8ec11d242b123dc9b";
pub var client_sample: [16]u8 = undefined;

const client_mask_hex = "437b9aec36";
pub var client_mask: [5]u8 = undefined;

const client_header_protected_hex = "" ++
    "c000000001088394c8f03e5157080000449e7b9aec34";
pub var client_header_protected: [client_header_protected_hex.len / 2]u8 = undefined;

const client_packet_hex = "" ++
...
pub var client_packet: [client_packet_hex.len / 2]u8 = undefined;

pub fn loadTestData() !void {
    _ = try std.fmt.hexToBytes(&client_body, client_body_hex);
    _ = try std.fmt.hexToBytes(&client_sample, client_sample_hex);
    _ = try std.fmt.hexToBytes(&client_mask, client_mask_hex);
    _ = try std.fmt.hexToBytes(&client_header_protected, client_header_protected_hex);
    _ = try std.fmt.hexToBytes(&client_packet, client_packet_hex);
}
